...
- **每日打卡**：支持早起、睡眠、用餐和运动打卡，记录每一天的生活习惯，时间分布可视化
- **今日待办**：管理每日任务，支持添加、编辑、删除和标记完成
- **未来倒计时**：创建重要事件的倒计时，时刻提醒用户珍惜时间
- **个人中心**：管理用户资料，支持头像上传、修改密码和账户注销
- **设置**：个性化应用设置，包括主题、字体大小、背景图片和反馈提交
...
- **user**：用户表（存储用户基本信息、密码加密、角色权限）
- **checkin**：打卡表（支持早起、睡眠、用餐、运动四种类型，含子类型）
- **todo**：待办表（支持优先级、完成状态、日期管理）
- **countdown**：倒计时表（支持自定义颜色和排序）
- **feedback**：反馈表（用户意见反馈和联系方式）
- **user_setting**：用户设置表（主题、字体、背景等个性化设置）
- **ai_chat**：AI对话记录表（存储与AI的对话历史）
